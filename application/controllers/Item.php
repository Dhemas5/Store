...
class Item extends CI_Controller
{
    ...
    function get_ajax()
    {
        ...
        foreach ($list as $item) {
            ...
            $row = array();
            $row[] = $no . ".";
            $row[] = $item->barcode . '<br><a href="' . site_url('index.php/item/barcode_qrcode/' . $item->item_id) . '" class="btn btn-default btn-xs">Generate <i class="fa fa-barcode"></i></a>';
            $row[] = $item->name;
            ...
            // add html for action
            $row[] = '<a href="' . site_url('index.php/item/edit/' . $item->item_id) . '" class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i> Update</a>
                    <a href="' . site_url('index.php/item/del/' . $item->item_id) . '" id="btn-hapus" class="btn btn-danger btn-xs"><i class="fa fa-trash"></i> Delete</a>';
            $data[] = $row;
        }
        $output = array(
            "draw" => @$_POST['draw'],
    ...
    public function edit($id)
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            echo "window.location = '" . site_url('index.php./item/') . "';</script>";
        }
    }
    ...
    public function process()
    {
        ...
        if (isset($_POST['add'])) {
            ...
            if ($this->item_m->check_barcode($post['barcode'], $post['id'])->num_rows() > 0) {
                $this->session->set_flashdata('error', "Barcode $post[barcode] Sudah Dipakai Barang Lain");
                redirect('index.php/item/edit/' . $post['id']);
            } else {
                if (@$_FILES['image']['name'] != null) {
                    if ($this->upload->do_upload('image')) {
